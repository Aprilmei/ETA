...
def predict_time(origin_id, destination_id, weekday, hour, jpid):
    ...
    origin_distance_list = conn.execute(
        "SELECT position_on_route FROM routes WHERE stop_id = {} AND journey_pattern = {};".format(origin_id, "'" + jpid + "'"))
    destination_distance_list = conn.execute(
        "SELECT position_on_route FROM routes WHERE stop_id = {} AND journey_pattern = {};".format(destination_id, "'" + jpid + "'"))
